...
#         'avg_color_r': float(avg_color[0]),
#         'avg_color_g': float(avg_color[1]),
#         'avg_color_b': float(avg_color[2]),
#         'frame_count': frame_count,
#         'keywords': ' '.join(keywords),
#         'sentiment': float(sentiment),
#         **{f'topic_{i}': topic_distribution[i] if i < len(topic_distribution) else 0 for i in range(10)}  # トピックの数に応じて修正
#     }

#     return features

# # トピックモデリングの準備
# all_texts = ' '.join(text_data['テキスト列'])
# tokenized_texts = [preprocess_text(text) for text in text_data['テキスト列']]
